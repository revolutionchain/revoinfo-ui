<template>
  <div v-show="addresses.length">
    <button class="button is-revo" @click="show = !show">
    ...
    <table v-show="show">
      <tbody>
        <tr v-for="{address, balance} in list">
          ...
          <td class="monospace has-text-right">{{ balance | revo(8) }}</td>
          <td>
      ...
      <tfoot v-if="addresses.length > 1">
        <tr>
          ...
          <td class="monospace has-text-right">{{ totalBalance | revo(8) }}</td>
          <td></td>
        </tr>
      </tfoot>
    </table>
    ...
    methods: {
      ...
      async onTransaction({address, id}) {
        ...
        if (window.Notification && Notification.permission === 'granted') {
          ...
          let options = {
            body: id,
            icon: 'https://upload.wikimedia.org/wikipedia/commons/thumb/5/51/Revo_logo.svg/102px-Revo_logo.svg.png',
            data: id,
            tag: 'transaction/' + id
          }
